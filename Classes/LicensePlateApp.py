...
import pytesseract
from Classes.Detect_char import LicensePlateRecognizer  # Nhớ import lớp LicensePlateRecognizer của bạn
...
class LicensePlateApp:
    def __init__(self, root):
        ...
        # Tiêu đề ứng dụng
        self.title_label = tk.Label(root, text=Config.WINDOW_TITLE, font=("Arial", 18, "bold"), bg="#ECF0F1", fg="#2C3E50", bd=2, relief="solid")
        ...
        self.start_camera_button = tk.Button(self.camera_control_frame, text=Config.NAME_START_CAMERA_BUTTON, command=self.start_camera, font=("Arial", 12), bg="#3498DB", fg="white", width=15, height=2, bd=2, relief="groove")
        self.start_camera_button.grid(row=0, column=0, padx=5)

        self.stop_camera_button = tk.Button(self.camera_control_frame, text=Config.NAME_STOP_CAMERA_BUTTON, command=self.stop_camera, font=("Arial", 12), bg="#E74C3C", fg="white", width=15, height=2, bd=2, relief="groove")
        self.stop_camera_button.grid(row=0, column=1, padx=5)

        # Các nút chức năng
        self.capture_button = self.create_button(Config.NAME_CAPTURE_BUTTON, self.capture_image, "#2ECC71")
        self.record_button = self.create_button(Config.NAME_RECORD_BUTTON, self.record_video, "#9B59B6")
        self.upload_button = self.create_button(Config.NAME_UPLOAD_BUTTON, self.upload_image, "#F1C40F")
        self.upload_video_button = self.create_button("Tải video lên", self.upload_video, "#1ABC9C")
        self.toggle_video_button = self.create_button("Chạy video", self.toggle_video_playback, "#1ABC9C")


        # Thêm thanh trượt phóng to
        self.zoom_slider = Scale(self.control_frame, from_=1, to=3, orient=HORIZONTAL, resolution=0.01, label=Config.NAME_ZOOM_SLIDER, command=self.update_zoom, length=300)
       ...
       # Khung chứa nút làm nét/đặt lại
        ...
        btn_sharpen = ttk.Button(button_frame, text=Config.NAME_SHARPEN_BUTTON, command=self.sharpen_image, style='TButton')
        btn_sharpen.pack(side=tk.LEFT, padx=10)

        # Tạo button "Đặt lại ảnh" với style
        btn_reset = ttk.Button(button_frame, text=Config.NAME_RESET_BUTTON, command=self.reset_image, style='TButton')
        btn_reset.pack(side=tk.LEFT, padx=10)

        # Nút chứa nhận diện biển số 
        self.recognize_button = self.create_button(Config.NAME_RECOGNIZE_BUTTON, self.recognize_license_plate, "#E74C3C")
    ...
    def toggle_video_playback(self):
        if not self.video_playing:
            self.upload_video()  # Lần đầu bấm thì chọn file và chạy video
        else:
            self.video_paused = not self.video_paused
            if self.video_paused:
                self.status_label.config(text="Video đang tạm dừng.")
                self.toggle_video_button.config(text="Tiếp tục")
            else:
                self.status_label.config(text="Tiếp tục xử lý video...")
                self.toggle_video_button.config(text="Tạm dừng")
    ...
    def process_video(self, video_path):
        ...
        self.status_label.config(text="Đang xử lý video...")
        detected_texts = []
        ...
        while cap.isOpened() and self.video_playing:
            ...
            frame, license_plate_texts = self.plate_recognizer.detect_license_plate_and_text(frame)

            frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            image_pil = Image.fromarray(frame_rgb)
            image_pil = image_pil.resize((self.image_label.winfo_width(), self.image_label.winfo_height()), Image.Resampling.LANCZOS)
        ...
        self.video_playing = False
        self.video_paused = False
        self.status_label.config(text="Đã xử lý xong video.")
        self.toggle_video_button.config(text="Chạy video")
